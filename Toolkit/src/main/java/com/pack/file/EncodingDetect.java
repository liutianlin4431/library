...
class BytesEncodingDetect extends Encoding {
	...
	int gbk_probability(byte[] rawtext) {
		...
		for (i = 0; i < rawtextlen - 1; i++) {
			...
			if (rawtext[i] >= 0) {
				...
				dbchars++;
				if ((byte) 0xA1 <= rawtext[i] && rawtext[i] <= (byte) 0xF7 && // Original
																				// GB
																				// range
				...
				} else if ((byte) 0x81 <= rawtext[i] && rawtext[i] <= (byte) 0xFE && // Extended
																						// GB
																						// range
	...
	int big5plus_probability(byte[] rawtext) {
		...
		for (i = 0; i < rawtextlen - 1; i++) {
			...
			if (rawtext[i] >= 128) {
				...
				dbchars++;
				if (0xA1 <= rawtext[i] && rawtext[i] <= 0xF9 && // Original Big5
																// range
				...
				} else if (0x81 <= rawtext[i] && rawtext[i] <= 0xFE && // Extended
																		// Big5
																		// range
						((0x40 <= rawtext[i + 1] && rawtext[i + 1] <= 0x7E)
		...
		for (i = 0; i < rawtextlen - 1; i++) {
			if (rawtext[i] >= 0) { // in ASCII range
				...
				if (i + 3 < rawtextlen && (byte) 0x8E == rawtext[i] && (byte) 0xA1 <= rawtext[i + 1]
					...
					i += 3;
				} else if ((byte) 0xA1 <= rawtext[i] && rawtext[i] <= (byte) 0xFE && // Plane
																						// 1
						(byte) 0xA1 <= rawtext[i + 1] && rawtext[i + 1] <= (byte) 0xFE) {
					cnschars++;
					totalfreq += 500;
	...
	int iso_2022_kr_probability(byte[] rawtext) {
		...
		for (i = 0; i < rawtext.length; i++) {
			if (i + 3 < rawtext.length && rawtext[i] == 0x1b && (char) rawtext[i + 1] == '$'
	...
	int iso_2022_jp_probability(byte[] rawtext) {
		int i;
		for (i = 0; i < rawtext.length; i++) {
			if (i + 2 < rawtext.length && rawtext[i] == 0x1b && (char) rawtext[i + 1] == '$'
					&& (char) rawtext[i + 2] == 'B') {
				return 100;
			}
		}
